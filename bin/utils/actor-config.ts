...
//   3. ActorConfigFile -> DenormalizedActor[] (denormalizeConfig — one normalized shape for everyone)
//   4. DenormalizedActor -> ActorConfig       (loadActorConfig — merges in .actor/actor.json)
//
...
const describeIssue = (issue: ConfigIssue, rawActors: unknown[]): string => {
 ...
 */
export const loadActorConfig = async (entry: DenormalizedActor): Promise<ActorConfig> => {
    const { folder } = entry;
    const actorJsonPath = folder ? `${folder}/.actor/actor.json` : '.actor/actor.json';

    const actorJson = await safeReadJsonObjectFile(actorJsonPath);
...
export const readConfigFile = async (selection: { actors: string[]; ignore: string[] }): Promise<ActorConfig[]> => {
    const rawConfigFile = await readConfigFileContents();
    ...
    for (const entry of entries) {
        ...
        actorConfigs.push(await loadActorConfig(entry));
    }
