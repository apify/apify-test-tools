...
const ModeSelectionSchema = z.object({
    mode: z.enum(CONFIG_FILE_STRATEGY).default(CONFIG_FILE_STRATEGY.LEGACY),
});

function selectStrategy(body: Record<string, unknown>): StrategyParser {
    const parsed = ModeSelectionSchema.safeParse(body);
    if (!parsed.success) {
        throw new Error(z.prettifyError(parsed.error));
    }

    const { mode } = parsed.data;
    return CONFIG_FILE_STRATEGIES[mode];
}
