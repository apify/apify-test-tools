...
6. **Cosmetic JSON classification** — `.json` files inside the actor's own `folder` with only cosmetic schema changes (whitespace, key ordering) only trigger a release build.
